...
public class DragonslayerSwordspear extends Weapon {


    public DragonslayerSwordspear(Tier tier, float attack, float speed, float knockback, int poise, int durability, int enchantability, float movementSpeed, Properties properties) {
        super(tier, attack, speed, knockback, poise, durability, enchantability, movementSpeed, properties);
    }

    public DragonslayerSwordspear(Tier tier, float attack, float speed, float knockback, int poise, int durability, int enchantability, float movementSpeed, int maxTargets, Properties properties) {
        super(tier, attack, speed, knockback, poise, durability, enchantability, movementSpeed, maxTargets, properties);
    }

    public DragonslayerSwordspear(Tier tier, float attack, float speed, float knockback, int poise, int blood, int poison, int frost, int rot, int death, int fire, int holy, int durability, int enchantability, float movementSpeed, int maxTargets, Properties properties) {
    ...
    public void doLeftClickAction(Player playerIn, ItemStack stackIn) {
        if(CommonConfig.do_special_attacks.get()) {
            if (!playerIn.isUsingItem()) {
                ...
                WeaponAttackEntity entity = new WeaponAttackEntity(EntityInit.DRAGONSLAYER_SWORDSPEAR.get(), levelIn, pos, (float) Mth.atan2(pos.z - playerIn.getZ(), pos.x - playerIn.getX()));
