...
public class HunterAxeExtended extends TrickWeapon {
    public HunterAxeExtended(Tier tier, float attack, float speed, float reach, float knockback, int poise, int blood, int poison, int frost, int rot, int death, int fire, int holy,int serrated, int durability, int enchantability, float movementSpeed, int maxTargets, Properties properties) {
    ...
    public void doLeftClickAction(Player playerIn, ItemStack stackIn) {
        if(CommonConfig.do_special_attacks.get()) {
            ...
            WeaponAttackEntity entity = new WeaponAttackEntity(EntityInit.HUNTER_AXE_EXTENDED.get(), levelIn, pos, (float) Mth.atan2(pos.z - playerIn.getZ(), pos.x - playerIn.getX()));
