...
public class WeaponAttackEntity extends Entity {
   ...
   /* public WeaponAttackEntity(EntityType<?> e, Level level, Vec3 pos, float dmg, int poisedmg, int fire, float smite, float bane,int bleed, int poison, int rot, int frost, int death , int updateTickAnimation, int updateTickPosition, int attackTick, int finalTick, double playerDistance, double hitboxHeight, double inflatex,double inflatey,double inflatez,float rotation) {
    ...
    protected void defineSynchedData() {
        this.getEntityData().define(ANIMATIONSTATE, 0);
        ...
        if (target.isAlive() && !target.isInvulnerable() && target != livingentity) {
            if(this.maxTargets<=0||this.hitEntities<=maxTargets) {
                ...
                if(this.fire>=1){
                        ...
                        target.setRemainingFireTicks(target.getRemainingFireTicks()+40 * fire);
                ...
                if(this.rot>=1){
                    target.addEffect(new MobEffectInstance(EffectInit.ROT.get(),40+this.rot*40,this.rot-1), this.getOwner());
                }
                if(this.toxic>=1){
                    target.addEffect(new MobEffectInstance(EffectInit.TOXIC.get(),40+this.toxic*40,this.toxic-1), this.getOwner());
                }
                if(this.frost>=1){
                    target.addEffect(new MobEffectInstance(EffectInit.FROST.get(),40+this.frost*40,this.frost-1), this.getOwner());
                }
    ...
    public void damageWeapon(){
        getItemStack().hurtAndBreak(1, getOwner(), (p_43276_) -> {
            p_43276_.broadcastBreakEvent(getItemStack().getEquipmentSlot());
        });
