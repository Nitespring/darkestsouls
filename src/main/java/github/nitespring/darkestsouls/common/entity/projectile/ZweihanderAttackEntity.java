...
public class ZweihanderAttackEntity extends Entity{
	@Nullable
	...
	private static final EntityDataAccessor<Integer> ANIMATIONSTATE = SynchedEntityData.defineId(ZweihanderAttackEntity.class, EntityDataSerializers.INT);

	public ZweihanderAttackEntity(EntityType<?> p_19870_, Level p_19871_) {
		super(p_19870_, p_19871_);

	}
	public ZweihanderAttackEntity(EntityType<?> p_19870_, Level p_19871_, Vec3 pos, float dmg, float rot) {
		 ...
		 public void tick() {
			...
			if(lifeTicks%3==0) {
			 ...
			 if(this.owner!=null&&lifeTicks%3==0) {
				 Vec3 pos = this.owner.position().add(this.owner.getLookAngle().x() * 1.9, 0.4, this.owner.getLookAngle().z() * 1.9);
				 this.setPos(pos);
		      ...
		      if (p_36945_.isAlive() && !p_36945_.isInvulnerable() && p_36945_ != livingentity) {
				  if(this.hitEntities<=maxTargets) {
					  ...
					  if (p_36945_ instanceof DarkestSoulsAbstractEntity && this.itemStack!=null && this.getOwner()!=null){
						  ((DarkestSoulsAbstractEntity) p_36945_).damagePoiseHealth((int) ((((Weapon)this.itemStack.getItem()).poiseDmgModifier-1)*((Weapon)this.itemStack.getItem()).getAttackDamage((Player) this.getOwner(),this.itemStack)));
					  }
					  this.hitEntities++;
				  }
