...
@Override
    protected void positionRider(Entity e, MoveFunction f) {
        if (this.hasPassenger(e)) {
            if(this.aimVec!=null) {
                e.setPos(new Vec3((float) (-aimVec.normalize().x()*0.6f), 1.0f, (float) (-aimVec.normalize().z()*0.6f)));
            }else {
                e.setPos(new Vec3((float) (-this.getLookAngle().normalize().x()*0.6f), 1.0f, (float) (-this.getLookAngle().normalize().z()*0.6f)));
            }
        }
