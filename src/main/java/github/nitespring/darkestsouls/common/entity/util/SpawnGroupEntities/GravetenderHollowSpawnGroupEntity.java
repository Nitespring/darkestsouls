...
public class GravetenderHollowSpawnGroupEntity extends SpawnGroupEntity{
    ...
    @Override
    public void doMobSpawns(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData, CompoundTag tag) {
        Random r = new Random();
        ...
        for(int i=0; i<=r.nextInt(3+size*2)+r.nextInt(3)-2;i++){
            ...
            mob.finalizeSpawn(level, difficulty, spawnType, spawnGroupData, tag);
